...
def delete_notification_by_key(user, unique_key):
    Notification.objects.filter(user=user, unique_key=unique_key).delete()
...
def sync_task_notifications(user):
    ...
    stale_notifications = Notification.objects.filter(user=user)

    for notification in stale_notifications:
        if notification.unique_key:
            if any(notification.unique_key.startswith(prefix) for prefix in stale_prefixes):
                if notification.unique_key not in active_keys:
                    notification.delete()
...
def sync_gamification_notifications(user):
    ...
    level_key = f"level-current-{profile.level}"
    keys_that_should_exist.add(level_key)
    create_or_update_unique_notification(
        user=user,
        type="level_up",
        title=f"Nível atual: {profile.level}",
        description="Continue focando para evoluir de nível e desbloquear novas recompensas.",
        priority="low",
        unique_key=level_key,
        metadata={"level": profile.level, "generated_at": now_iso},
        expires_at=None,
    )

    if profile.daily_goal_progress == 0:
        key = "no-focus-today"
        keys_that_should_exist.add(key)
