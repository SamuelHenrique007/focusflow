...
def is_user_offline(
    ...
    *,
    reference_dt=None,
    offline_threshold_minutes: int = OFFLINE_THRESHOLD_MINUTES,
) -> bool:
    ...
    if timezone.is_naive(last_seen_at):
        last_seen_at = timezone.make_aware(
            last_seen_at,
...
def get_tasks_due_soon_for_email(
    user,
    *,
    reference_dt=None,
    window_minutes: int = DUE_SOON_WINDOW_MINUTES,
):
    reference_dt = reference_dt or timezone.now()
...
def get_tasks_became_overdue_for_email(
    user,
    *,
    reference_dt=None,
    window_minutes: int = BECAME_OVERDUE_WINDOW_MINUTES,
):
    reference_dt = reference_dt or timezone.now()
...
def build_weekly_productivity_summary(user, reference_dt=None):
    ...
    focus_minutes = (
        completed_focus_sessions.aggregate(total=Sum("planned_minutes"))["total"] or 0
    )

    focus_sessions_count = completed_focus_sessions.count()
    completed_tasks_count = completed_tasks.count()
    average_focus_minutes = (
        round(focus_minutes / focus_sessions_count)
        if focus_sessions_count
        else 0
    )
    ...
    return {
        ...
        "next_recommended_action": (
            most_recent_task.title
            if most_recent_task
            else "Revisar prioridades da próxima sessão"
        ),
...
def send_productivity_summary_email(user, summary: dict) -> None:
    context = {
        **_base_context(user),
        **summary,
        "summary_weekday": WEEKDAY_LABELS.get(
